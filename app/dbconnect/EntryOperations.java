...
public class EntryOperations {
    ...
    public ImmutableList<Entry> getEntries(String parent) {
        final String sql = "SELECT rowid, title, slug FROM entries WHERE parent = '"+parent+"' ORDER BY time DESC;";
     ...
     */
    public String createEntry(String parent, String slug) {
        if (slug.isEmpty()||parent.isEmpty()) {
        ...
        final String insertSql = "INSERT INTO entries (parent, slug, time) VALUES ('" + entry.getParent() + "', '" +
                entry.getSlug() +
                "', " +
                timestamp.getTime() +
                ");";
